...
**Offen (nach M5):** Supabase-MCP war die gesamte Session nicht verbunden → Migration `m5_deals_contracts` noch NICHT auf die Live-DB angewendet (via `apply_migration` nachziehen, danach `get_advisors` prüfen) und der volle Browser-E2E (NordSport schlägt Deal vor → Lena Gegenangebot/Zustimmung → beidseitig bestätigt → Status `agreed`) steht noch aus. Deal-Notifications zählen noch nicht ins Header-Badge (bleibt Nachrichten-only bis zum Notification-Center in M7). Meilenstein-Submit/Approve + funded/Zahlungen folgen mit M6 (Stripe).
