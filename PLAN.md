...
**Offen (nach M4):** Supabase-MCP war in der Session nicht verbunden → Migration `20260704090000_m4_conversations_messages.sql` noch NICHT live angewendet (via `apply_migration` nachziehen, danach `get_advisors` prüfen — erwartete neue WARNs: keine; `handle_new_message` ist bewusst SECURITY DEFINER ohne Execute-Grant). Danach voller Browser-E2E: als Lena Konversation zu einem NordSport-Listing starten, als NordSport antworten, Realtime + Ungelesen-Zähler beidseitig prüfen. Notifications-Anzeige bleibt minimal (Badge = ungelesene Nachrichten; Ausbau in M7).
