...
/**
 * SponsorMatch — MatchCard: die Marktplatz-Ergebniskarte eines Gesponserten.
 * Cover ohne Bild = Marken-Gradient navy→teal (einziger erlaubter Gradient).
 */
export interface MatchCardStat {
  value: string;
  label: string;
}
    ...
    >
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            cursor: "pointer",
            background: "var(--surface)",
            color: isSaved ? "var(--energy)" : "var(--text-muted)",
            display: "inline-flex",
            alignItems: "center",
      ...
      <div style={{ padding: "0 var(--space-5) var(--space-5)", marginTop: -26 }}>
        ...
        <div style={{ marginTop: 10, display: "flex", alignItems: "center", gap: 7 }}>
          <span style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "var(--fs-h4)", color: "var(--text)", letterSpacing: "-0.01em" }}>
            {name}
          </span>
        ...
        {rating != null && (
          <div style={{ marginTop: 10 }}>
            <RatingStars value={rating} count={ratingCount} size={15} showValue />
          </div>
        )}
          ...
          <div style={{ display: "flex", gap: 18, marginTop: 14, paddingTop: 14, borderTop: "1px solid var(--border)" }}>
            {stats.map((s, i) => (
              <div key={i} style={{ display: "flex", flexDirection: "column" }}>
                ...
                <span style={{ fontSize: "var(--fs-2xs)", color: "var(--text-muted)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em" }}>
                  {s.label}
                </span>
              </div>
            ))}
        ...
        {tags.length > 0 && (
          <div style={{ display: "flex", gap: 6, marginTop: 14, flexWrap: "wrap" }}>
            {tags.map((t, i) => (
              <Tag key={i} style={{ padding: "3px 9px", fontSize: "var(--fs-xs)" }}>
                {t}
              </Tag>
            ))}
          </div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginTop: 16 }}>
          ...
          <Button variant="primary" size="sm" onClick={onView} style={{ marginLeft: "auto" }}>
            Profil ansehen
          </Button>
        </div>
      </div>
