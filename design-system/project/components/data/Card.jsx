...
/** SponsorMatch — Card: surface container with optional hover lift & accent. */
export function Card({ children, padding = 'md', interactive = false, accent, header, footer, style, onClick, ...rest }) {
  ...
  return (
    <div
      ...
      {...rest}
    >
      {header && <div style={{ padding: `${pad === 0 ? 'var(--space-5)' : pad} ${pad === 0 ? 'var(--space-5)' : pad} 0` }}>{header}</div>}
      <div style={{ padding: pad, flex: 1 }}>{children}</div>
      {footer && <div style={{ padding: pad, paddingTop: 0, borderTop: '1px solid var(--border)', background: 'var(--surface-inset)' }}>{footer}</div>}
    </div>
  );
}
