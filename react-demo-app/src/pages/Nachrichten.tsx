...
import { getConversationOverviews, type ConversationOverview } from "@/db/repositories/conversations";
import { formatMessageTimestamp } from "@/lib/format";

/** Konversationsliste — Struktur 1:1 aus app/(app)/nachrichten/page.tsx. */
export default function Nachrichten() {
  const { profile } = useSession();
  const [overviews, setOverviews] = React.useState<ConversationOverview[]>([]);

  React.useEffect(() => {
    if (!profile) return;
    void (async () => {
      const db = await getDb();
      setOverviews(getConversationOverviews(db, profile.id));
    })();
  }, [profile]);
      ...
      {overviews.length === 0 ? (
        <Card padding="lg">
          <p style={{ margin: 0, color: "var(--text-muted)" }}>
            Noch keine Konversationen — starte eine über den Kontakt-Button auf einem Listing oder Profil, das zu dir passt.
          </p>
          <p style={{ margin: "var(--space-3) 0 0" }}>
            <Link to="/suche" style={{ fontWeight: 600, color: "var(--primary)" }}>
        ...
        <Card padding="none">
          <ul style={{ listStyle: "none", margin: 0, padding: 0 }}>
            {overviews.map(({ conversation, counterpart, listingTitle, lastMessage, unreadCount }, i) => (
              <li key={conversation.id} style={{ borderTop: i > 0 ? "1px solid var(--border)" : "none" }}>
                <Link
